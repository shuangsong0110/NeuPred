#' @import data.table
#' @import utils
processLD <- function(summs,testpath=NULL,LDpath,LDpath.chr=NULL,pos,path=NULL,plinkpath,chr){
  ...
  if(!is.null(LDpath)){
    ...
    for(chr0 in chr){
      ...
      bed$stop[nrow(bed)-1] <- bed$stop[nrow(bed)]
      bed <- bed[-nrow(bed),]
